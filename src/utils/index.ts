 ...
 */
export function hasScrollElement(el: HTMLElement, direction = "vertical") {
  if(!el) return
 ...
 * @param {String} direction 
 */
export function getFirstScrollElement(el: HTMLElement, direction = "vertical"): HTMLElement | void {
  if(!el) return
  if (hasScrollElement(el)) {
    return el
  } else {
    return getFirstScrollElement(el ?? (el as HTMLElement).parentElement, direction)
  }
}
 ...
 * @param {Boolean} full 
 */
export function fromatTime(time: Date, full = true) { //
 ...
 * @param {String} type 
 */
export function formatDate(time: Date, type = 'YYYY-MM-DD'): string | number | Date { //
  const y = formatNumber(time.getFullYear())
  const m = formatNumber(time.getMonth() + 1)
  const d = formatNumber(time.getDate())
  const h = formatNumber(time.getHours())
 ...
 */
export const formatDateToZH = (date: string | number) => {
 ...
 * validate reg
 */
export const validateNickname = (rule: Rules, value: string, callback: Function) => {
  if (value === "") {
    callback();
  } else {
    if (value.length > 12) {
...
export const validatePhone = (rule: Rules, value: string, callback: Function) => {
  if (value === "") {
    callback();
  } else {
...
export const validateEmail = (rule: Rules, value: string, callback: Function) => {
  if (value === "") {
    callback();
  } else {
...
export const validateSignature = (rule: Rules, value: string, callback: Function) => {
  if (value === "") {
    callback();
  } else {
    if (value.length > 100) {
...
export const saveMyFriendsToLocalStorage = (data: unknown) => {
  window.localStorage.setItem('friends', JSON.stringify(data))
}
export const saveMyGroupToLocalStorage = (data: unknown) => {
  data = JSON.stringify(data)
  window.localStorage.setItem('groups', data = JSON.stringify(data))
}
 ...
 */
// bug-1 fix data type
export const saveRecentConversationToLocal = (data: any) => {
 ...
 */
// bug-2 refactor T
export const arrUnique = <T>(arr: Array<T>) => {
  const list = [...arr] as Array<T>
  return list.reduce((mult: Array<T>, item) => {
 ...
 */
export const findParentNode = (target: HTMLElement | Document, parentClassName: string) => {
  let cur = target
  while (cur !== null && cur !== document) {
    if ((cur as HTMLElement).classList.contains(parentClassName)) {
      return cur
    }
    cur = cur ?? (cur as HTMLElement).parentNode
  }
}
...
export class LocalStorageManager {
  ...
  set(key: string, value: unknown) {
    ...
    window.localStorage.setItem(key, strVal as string)
  }
}
 ...
 */
export const localImgToBase64 = (fileDom: HTMLInputElement) => {
  /**jpeg和jpg是一种 */
  const imgTypeArr = ['jpeg', 'png', 'gif', 'jpg']
  return new Promise((resolve, reject) => {
    if (!fileDom.files) return
    const [file] = fileDom.files as FileList
...
export function colorRgb (str: string) {
 ...
 export function computedLevel(onlineTime: number) {
